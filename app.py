    ...
    # ==========================================
        ...
        with tab_d1:
            ...
            if not filtered_demo.empty:
                ...
                # 📌 批次勾選匯出功能
                st.markdown("#### 📦 批次勾選匯出")
                batch_opts_d = [f"{r['Log_ID']} (客戶:{r['Customer']} | 機台:{r['Equipment']})" for idx, r in filtered_demo.iterrows()]
                selected_batch_d = st.multiselect("請選擇要一起匯出的實驗紀錄 (可多選，支援關鍵字快速搜尋)：", batch_opts_d)
                
                if selected_batch_d:
                    batch_ids_d = [opt.split(" ")[0] for opt in selected_batch_d]
                    batch_df_d = filtered_demo[filtered_demo['Log_ID'].isin(batch_ids_d)]
                    csv_batch_d = batch_df_d.to_csv(index=False).encode('utf-8-sig')
                    st.download_button(
                        label=f"📥 點此下載您勾選的 {len(batch_ids_d)} 筆實驗報告", 
                        data=csv_batch_d, 
